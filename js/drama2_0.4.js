...
/*            let pixelRatio = this.bs.o3d.renderer.getPixelRatio();
        ...
        _initStage(){
            ...
            ta.roofInOut=new TimelineMax({paused:true,delay:1,onStart:()=>{this._roofHideOrShow()},onUpdate:()=>{this.status.taPlaying=true;},onComplete:()=>{this.status.taPlaying=false;},onReverseComplete:()=>{this.status.taPlaying=false;this._roofHideOrShow(false)}});
        ...
        _initStageConfig(child){
            ...
            }else if(child.name==="main_dm")
            {
                ...
                this.sc=child;
            }else if(child.name.split("_")[0]==="adorn"){
/*                let name=child.userData.model;
