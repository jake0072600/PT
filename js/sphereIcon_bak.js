let SphereIcon = (function () {
    ...
    class SphereIcon{
        ...
        init(){
            ...
            mesh.position.copy(p.clone().multiplyScalar(1.005));
            ...
            og.position.copy(p.clone().multiplyScalar(1.005));
            og.rotation.copy(r);
            this.object3d.add(og);
            this.og=og;
    ...
    class SphereIconFire extends SphereIcon{
        constructor(img,object3d,point,size,options) {
            ...
            if(options.fireOffset){
                fg.position.set(0,0,-options.fireOffset)
            }
            if(options.model){
                this.model=new THREE.Group();
                this.model.add(options.model);
                this.og.add(this.model);
            }
        ...
        setFireHigh(scaleY){
            let fg=this.fg;
            let tg = TWEEN.SphereIcons[this.id];
            fg.userData.tween.stop();
            tg.remove(fg.userData.tween);

            fg.userData.tween=null;
            delete fg.userData.tween;

            fg.userData.tween=new TWEEN.Tween(fg.scale,tg).to({y:scaleY,x:1.5,z:1.5},500+Math.random()*500).repeat(Infinity).yoyo(true).start();
        }
    }
